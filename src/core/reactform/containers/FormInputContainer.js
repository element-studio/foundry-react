    ...
    printErrors(){
        ...
        for (let i = 0; i < failedValidationRules.length; i++ ){
            if(validationMessages.hasOwnProperty(failedValidationRules[i])){
                newErrArr.push(<span className="form-error-message" key={i}>{validationMessages[failedValidationRules[i]]}</span>);
            }else{
                newErrArr.push(<span className="form-error-message" key={i}>{failedValidationRules[i]}</span>);
            }
        }
        ...
        for (let i = 0; i < failedServerValidationRules.length; i++ ){
            ...
            if(validationMessages.hasOwnProperty(srvError)){
                newErrArr.push(<span className="form-error-message" key={srvError}>{validationMessages[srvError]}</span>);
            }else{
                newErrArr.push(<span className="form-error-message" key={srvError}>{srvError}</span>);
            }
        }
